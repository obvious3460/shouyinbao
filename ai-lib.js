...
- sales 销售单：id, no(单号), time(时间戳,毫秒), subtotal(小计), manual_discount_amt(整单折扣), vip_discount(VIP折扣), points_value(积分抵扣), payable(实收金额), pay_method(支付方式), cashier(收银员), employee_id, employee_name, member_id, member_name, member_level, points_earned(本次获得积分)
...
- employees 员工：id, username, name, active, created_at（没有工时字段，不能据此断定员工效率）
- expenses 出账：time(时间戳,毫秒), category(类别), amount(金额), note(备注), employee_id, employee_name
...
3. 不需要查数据时，直接用简体中文回答。
4. 经营口径：营业额=SUM(sales.payable)；客单价=营业额/订单数；商品成本=SUM(sale_items.cost*qty)；估算毛利=营业额-商品成本；估算净收益=估算毛利-expenses.amount。必须说明这是基于已登记数据的估算，不能把标价销售额冒充实收营业额。
5. 商品与销售单通过 sale_items.sale_id=sales.id 关联；商品与库存通过 sale_items.product_id=products.id 关联。按员工统计优先使用 sales.employee_name，为空时可使用 cashier。
6. 不要编造数据库没有的信息（例如员工工时、天气、客流、供应商价格）。会员手机号属于敏感信息，除非用户明确要求查看，否则不要选择 phone 字段。
7. 金额默认是人民币元。回答用简体中文，先给结论，再给关键数字和可执行建议；数据不足时明确说明。`;
}
...
function detectChartIntent(msg){
  ...
  if (/明细|每笔|逐笔|具体记录|消费记录|交易记录|清单|列表|全部记录|每一笔|记账明细/.test(m)) return 'text';
